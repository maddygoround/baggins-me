...
> = ({ title, preview, images, featured, slug, types, className }) => (
  <Link href={`/[projectSlug]`} as={`/${slug}`}>
    ...
    >
      {/* {featured && (
        <div className="pb-3/3 bg-gray-100 relative border-b overflow-hidden">
          {images && images[0] && (
            <img
              className={clsx("absolute w-small h-small object-cover")}
...
export const Work: React.FC<{
  ...
  <div className="container my-8">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-4">
      {projects.slice(0, preview ? 7 : undefined).map(p => (
